...
func Generate(components []rc.Appender, namesRegistry NamesRegistry, currentDir string) (string, error) {
	...
	file := newRepository(repositoryPackageName, namesRegistry)

	for _, c := range components {
		c.AppendTo(file)
	}

	repositoryFullPath := makeRepositoryDir(namesRegistry.FileName, currentDir, repositoryPackageName)
	return repositoryFullPath, file.Save(repositoryFullPath)
}

func newRepository(repositoryPackageName string, namesRegistry NamesRegistry) *j.File {
...
func makeRepositoryDir(fileName, currentDir, repositoryPackageName string) string {
